<template>
  <div class="app-page">
      ...
      <div class="app-form">
        <el-form ref="form" :inline="true" label-width="60px" label-position="left">
          <el-form-item label="姓名:">
            <el-input v-model="listQuery.teacherName" placeholder="请输入关键字查询" />
          </el-form-item>
          <el-form-item label="毕业/现读学校:" label-width="120px">
            <el-input v-model="listQuery.school" placeholder="请输入关键字查询" />
          </el-form-item>
          <el-form-item label="注册来源:" label-width="90px">
            <el-select v-model="listQuery.channel" size="small" placeholder="请选择">
              <el-option v-for="item in channelOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="手机号:">
            <el-input v-model="listQuery.phone" placeholder="请输入关键字查询" />
          </el-form-item>
      ...
      <el-table v-loading="listLoading" :data="list" border empty-text="暂无数据">
        ...
        <el-table-column v-for="(item, jndex) in columnList" :key="jndex" :prop="item.prop" :label="item.label" :width="item.width" show-overflow-tooltip :fixed="item.fixed" align="left">
          <template slot-scope="scope">
            <span v-if="item.prop == 'channel'">{{ scope.row.channel == 1 ? 'PC' : (scope.row.channel == 2 ? '微信小程序' : '--') }}</span>
    ...
    <teacherAdd :isAddDrawer="isAddDrawer" ref="teacherAdd" :type="type" @addClose="addClose" />
  </div>
</template>

<script>
import Pagination from "@/components/Pagination";
import { getTeacherList, deleteTeacher } from "@/api/operate.js";
import teacherAdd from "../teacher/edit";
const params = {
  teacherName: "",
  school: "",
  phone: "",
...
export default {
  name: "teacherList",
  components: { Pagination, teacherAdd },
  data() {
    return {
      total: 0,
      ...
      isAddDrawer: false,
      columnList: [
        { prop: "teacherName", label: "教员姓名" },
        { prop: "channel", label: "注册来源" },
  ...
  methods: {
    getList() {
      ...
      return getTeacherList(this.listQuery)
        .then(response => {
    ...
    handleClick(type, row) {
      ...
      if (type === "look") {
        ...
        this.id = row.id;
        this.$refs.teacherAdd.init(row.id)
      } else if (type == "add") {
        this.isAddDrawer = true;
        this.$refs.teacherAdd.getListGradeInfo()
      } else if (type === "edit") {
        this.isAddDrawer = true;
        this.id = row.id;
        this.$refs.teacherAdd.init(row.id)
